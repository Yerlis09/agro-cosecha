document.addEventListener('DOMContentLoaded', (event) => {
    const detectorId = document.getElementById('document_id').value;
    const mapDiv = document.getElementById('mapDiv');

    // Hacer todos los inputs de solo lectura
    const inputs = document.querySelectorAll('#detector-form input');
    inputs.forEach(input => {
        input.readOnly = true;
    });
    
    cargarZonaYParcelasGuardadas(mapDiv);
    obtenerSensores(detectorId);
});

const modalSensor = document.getElementById("btn-sensor"),
    myModalRegistrar = document.getElementById("myModalSensor"),
...
if (btn_guardar) {
    btn_guardar.addEventListener('click', function () {
        document.getElementById('detector-form').submit();
    });
}

// Mostrar modal de registrar sensor o modal de error
if (modalSensor) {
    modalSensor.addEventListener("click", function (e) {
        e.preventDefault();
        const sensoresRealizados = parseInt(localStorage.getItem('sensores_realizados'), 10);
        const sensoresEstimados = parseInt(localStorage.getItem('sensores_estimados'), 10);

        if (sensoresRealizados >= sensoresEstimados) {
            $('#errorModalSensoresCompletados').modal('show');  // Mostrar un modal de error indicando que no se pueden crear más sensores
            return;
        }

        if (isParcelaSeleccionada()) {
            const identificadorUnico = generarIdentificadorUnico();
            document.getElementById('sensor').value = identificadorUnico;
...
function filtrarBitacora(inputId, listaId) {
    ...
    fetch("/detectores/filtrar_bitacoras", {
        method: 'POST',
        headers: {
...
function cargarZonaYParcelasGuardadas(mapDiv) {
    ...
    fetch(`/detectores/obtener_zona_por_bitacora/${idCuaderno}`)
...
function eliminarBitacora(inputId) {
    ...
    const btnEliminar = input.parentNode.querySelector('.btn-clear-sensor');
    if (btnEliminar) {
        btnEliminar.remove();
    }
}
...
function generarIdentificadorUnico() {
    let sensorCounter = parseInt(localStorage.getItem('sensorCounter')) || 0;
    sensorCounter += 1;
    localStorage.setItem('sensorCounter', sensorCounter);
    return `Sensor nro. ${sensorCounter}`;
}

function obtenerSensores(detectorId) {
    fetch(`/detectores/obtener_sensores/${detectorId}`)
    .then(response => response.json())
    .then(data => {
        if (data.sensores) {
            mostrarSensores(data.sensores, detectorId);
            // Guardar los sensores realizados y estimados en el local storage para usarlos en el evento click
            localStorage.setItem('sensores_realizados', data.sensores_realizados);
            localStorage.setItem('sensores_estimados', data.detectores_estimados);
        } else {
...
function mostrarSensores(sensores, detectorId) {
    ...
    sensores.forEach((sensor, index) => {
        ...
        sensorItem.classList.add('list-group-item', 'd-flex', 'justify-content-between', 'align-items-center');

        sensorItem.innerHTML = `
            <div>
                <i class='bx bxs-sensor-alt'></i> Sensor #${index + 1}<br>
                ${sensor.fecha_inicio} ${sensor.hora_inicio}
            </div>
            <button class="btn btn-success" id="btn-ver-sensor-${index}" data-index="${index}">Ver</button>
        `;

        listaSensores.appendChild(sensorItem);
    });

    // Agregar evento a cada botón de ver sensor
    sensores.forEach((sensor, index) => {
        const btnVerSensor = document.getElementById(`btn-ver-sensor-${index}`);
        btnVerSensor.addEventListener('click', (e) => {
            e.preventDefault();  // Evitar que se envíe el formulario
            abrirModalEditarSensor(sensor, detectorId);
        });
    });
}

function abrirModalEditarSensor(sensor, detectorId) {
    // Cargar la información del sensor en el formulario
    document.getElementById('sensor_actualizar').value = sensor.sensor;
    document.getElementById('fecha_inicio_actualizar').value = sensor.fecha_inicio;
    document.getElementById('hora_inicio_actualizar').value = sensor.hora_inicio;
    document.getElementById('hora_fin_actualizar').value = sensor.hora_fin;
    document.getElementById('observ_actualizar').value = sensor.observ;
    document.getElementById('listLayerId_actualizar').value = sensor.listLayerId;
    document.getElementById('listOutLayerId_actualizar').value = sensor.listOutLayerId;
    
    // Establecer la URL correcta en el formulario
    const form = document.getElementById('formActualizarSensor');
    form.action = `/detectores/actualizar_sensor/${detectorId}/${sensor.id}`;

    verParcelas(sensor);

    // Mostrar el modal
    $('#myModalActualizarSensor').modal('show');
}

function verParcelas(sensor) {
    ...
    // Luego, resaltar solo las parcelas específicas del sensor
    const parcelas = JSON.parse(sensor.listLayerId);
    const outLayers = JSON.parse(sensor.listOutLayerId);
    ...
    parcelas.forEach((layerId, index) => {
        ...
        // Agregar las parcelas específicas del sensor a la lista de parcelas seleccionadas
        parcelasSeleccionadas.add(layerId);
        listaDeOutLayerId.add(outlineLayerId);
    });
}
...
document.getElementById('periocidad').addEventListener('change', function () {
    ...
    if (periodicidades[periocidad] && duracion) {
        ...
        const numeroDeSensores = Math.floor(duracionDias / intervalo);

        document.getElementById('sensores_estimados').value = numeroDeSensores;
    }
});
